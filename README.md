...
After importing a valid workbook, use **Add system / package / activity / gate**, or select an existing element to edit it. Existing IDs remain read-only. Activity fields include nominal whole-hour duration, `WORK_TIME`/`ELAPSED_TIME`, interruptibility, system-arrival requirement and optional activity calendar. System arrival must include a timezone offset. New activities have no Resource or Zone demands until added explicitly in **Resources, zones and calendars**; review this before operational use.

The constraint editor manages role or pool capacities (never named personnel), Activity demands, Zone capacity and shared or exclusive occupation, Calendar timezone, validity dates and weekend days, and local weekday Shifts including overnight shifts. Select an Activity to narrow its demands. A proposed form change shows the resulting V1 validation errors before saving; saved changes revalidate the complete model and clear the old schedule. `ALL` requires exclusive Zone occupancy; identical shifts are rejected. Equal start and end times define a 24-hour shift. `weekend_days` is descriptive in V1 scheduling: enabled weekday shifts determine actual working slots. Calendar exceptions and Resource substitutions are outside this editor.

**Duplicate activity** copies its V1 fields and Resource/Zone demands with a new ID, without copying links. **Duplicate package** copies its activities, package Gates (except `PROJECT_COMPLETE` and project milestones), Resource/Zone demands and dependencies strictly within the package; it does not copy cross-package links or milestone priorities. Review the copied fields and connect new elements explicitly. Select an Activity or Gate to use **Add predecessor** and **Add successor**; supply the link ID, other endpoint, non-negative whole-hour lag and justification. Selecting a Gate shows its priority; **Manage all gate priorities** expands the full list. Each change runs the existing V1 validator; errors such as duplicate IDs, unknown references, enabled cycles, duplicate ranks or a missing `PROJECT_COMPLETE` rank disable calculation and revision download until resolved. Edits reset any previous result.

**Download new .xlsx revision** writes a separate file using the original workbook held in memory. It updates `Metadata` and only the changed V1 tables among `Systems`, `Packages`, `Activities`, `Gates`, `Dependencies`, `Resources`, `ActivityResources`, `Zones`, `ActivityZones`, `Calendars`, `CalendarShifts` and `MilestonePriorities`. Weekend days are written as comma-separated weekday codes and reimported as the V1 array. Existing cells are preserved when changes only append records or edit fields; `revision_id` receives a timestamp suffix, `parent_revision` refers to the imported revision, `revision_timestamp` is UTC ISO 8601, and an optional `revision_comment` can be supplied. The app reimports the output and checks all normalized records, unchanged worksheet values and formulas before offering the download. It does not overwrite the imported file. Excel features outside the V1 table model, such as embedded objects and advanced workbook features, are not covered by the preservation check; review such workbooks in Excel before using the revision.
