...
After importing a valid workbook, select an Activity or Gate in **Explore workbook**. Its direct dependencies can be edited, deactivated or removed beside the selected element; **Add predecessor** and **Add successor** prefill that endpoint. Supply the link ID, other endpoint, non-negative whole-hour lag and justification. Selecting a Gate shows its priority; **Manage all gate priorities** expands the full list, with enabled ranks first. Each change runs the existing V1 validator; errors such as duplicate IDs, unknown references, enabled cycles, duplicate ranks or a missing `PROJECT_COMPLETE` rank disable calculation and both downloads until resolved. Edits reset any previous result.
